...
async function query(filterBy = {}) {
  // Live categories for Home page
  if (filterBy?.txt === 'HomePage') {
    // Forward locale/country/limit/offset to backend
...
async function getById(categoryId, name) {
  try {
    // Try Spotify category playlists first (backend maps to card shape).
    // Pass category name as query param so server can fallback to search if needed.
    const items = await httpService.get(`playlist/category/${categoryId}`, name ? { name } : undefined)
    console.log('[Category] api/category playlists response', {
      categoryId,
      name,
    ...
    // Fallback: some category IDs return 404 from Spotify. In that case search by name.
    if (name) {
      console.log('[Category] fallback to search for', { name, categoryId })
      const searchItems = await httpService.post(`playlist/search`, { searchKey: name, searchType: 'playlist' })
      console.log('[Category] fallback search response', {
        name,
        count: Array.isArray(searchItems) ? searchItems.length : 0,
        sample: Array.isArray(searchItems) && searchItems[0],
      })
      return searchItems
    }
    return []
  } catch (err) {
    // On error, try search fallback if name present
    if (name) {
      try {
        console.log('[Category] error on category fetch, fallback search', { name, categoryId })
        const searchItems = await httpService.post(`playlist/search`, { searchKey: name, searchType: 'playlist' })
        console.log('[Category] fallback search (error branch) response', {
          name,
          count: Array.isArray(searchItems) ? searchItems.length : 0,
          sample: Array.isArray(searchItems) && searchItems[0],
        })
        ...
        console.log('[Category] fallback search failed', e)
      }
    }
    throw err
  }
